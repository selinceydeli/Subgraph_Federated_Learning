#!/usr/bin/env python3
"""
FedAvg with Cross-Client Embedding Bootstrap.

Ghost node features are augmented with pre-computed embeddings from
extract_local_embeddings.py before training begins. This gives each
client structural context about nodes it cannot directly observe.

Owned nodes receive a zero bootstrap vector (the model learns their
representation through training). Ghost nodes receive the embedding
produced by the owning client's locally-trained model.

Pre-requisites:
    1. python3 -m scripts.training.train_local_baseline   (produces checkpoints)
    2. python3 -m scripts.data.extract_local_embeddings --num_clients N

Usage:
    python3 -m scripts.training.train_fedavg_with_bootstrap
"""
...
def augment_with_bootstrap(client_data, emb_dict, hidden_dim):
    """
    Augment ghost node features with pre-computed embeddings.

    Owned nodes get a zero vector (they learn their own representation).
    Ghost nodes get their embedding from the owning client's canonical model.

    x shape changes from [N, in_dim] to [N, in_dim + hidden_dim].
    """
    N = client_data.num_nodes
    aug = torch.zeros(N, hidden_dim, dtype=torch.float32)
    owned_mask = client_data.owned_mask  # [N] bool
    gnids = client_data.global_nid       # [N] int64
    for i in range(N):
        if not owned_mask[i].item():     # ghost node
            gnid = int(gnids[i].item())
            if gnid in emb_dict:
                aug[i] = emb_dict[gnid]
    if client_data.x is None:
        client_data.x = aug
    else:
        client_data.x = torch.cat([client_data.x, aug], dim=-1)  # [N, in_dim + hidden_dim]
    return client_data


def make_eval_loader(client_data, task, device, shuffle=False):
    """
    Preprocess an augmented val/test client subgraph and build a loader for evaluation.
    Mirrors train_fedavg.py's make_eval_loader but operates on already-augmented data.
    """
    data = check_and_strip_self_loops(client_data, "eval")
    # Note: client_data.x is already augmented; ensure_node_features must not overwrite it
...
def main():
    ...
    partition = "partition_aware_with_cross_edges" if args.include_cross_edges else "partition_aware_without_cross_edges"

    print(f"[Config] global_epochs={args.global_epochs}, local_epochs={args.local_epochs}, "
          f"client_fraction={args.client_fraction}, num_clients={num_clients}, "
    ...
    print(f"[Bootstrap] hidden_dim={HIDDEN_DIM}, partition={partition}")

    # Resolve data directories
    train_dir, val_dir, test_dir = resolve_data_dirs(partition_cfg)
    print(f"[Data] train_dir={train_dir}")

    # Load all client data (original, un-augmented)
    ...
    # --- Global port vocab precomputation (from original, un-augmented data) ---
    ...
    data_dir = "./data"
    emb_base_dir = f"./data/local_embeddings/{partition}/{num_clients}_clients"
    ...
    for seed in seeds:
        print(f"\n{'='*60}")
        print(f"[Seed {seed}] Starting Bootstrap FedAvg training ({num_clients} clients)...")
        print(f"{'='*60}")

        # --- Load pre-computed embeddings for this seed ---
        emb_path = os.path.join(emb_base_dir, f"seed{seed}", "node_embeddings.pt")
        if not os.path.exists(emb_path):
            raise FileNotFoundError(
                f"Embeddings not found at {emb_path}.\n"
                f"Run first: python3 -m scripts.data.extract_local_embeddings --num_clients {num_clients}"
            )
        emb_dict = torch.load(emb_path, map_location="cpu", weights_only=False)
        print(f"[Bootstrap] Loaded {len(emb_dict)} pre-computed node embeddings from {emb_path}")

        # --- Augment all splits for all clients ---
        # deepcopy to avoid modifying the originals (which are reused across seeds)
        train_list_aug = [
            augment_with_bootstrap(copy.deepcopy(d), emb_dict, HIDDEN_DIM)
            for d in train_list
        ]
        val_list_aug = [
            augment_with_bootstrap(copy.deepcopy(d), emb_dict, HIDDEN_DIM)
        ...
        # Verify augmented feature dimension
        aug_dim = train_list_aug[0].x.size(-1)
        assert aug_dim == 1 + HIDDEN_DIM, (
            f"Expected augmented x dim={1 + HIDDEN_DIM}, got {aug_dim}"
        )
        print(f"[Bootstrap] Augmented feature dim: {aug_dim} (= 1 orig + {HIDDEN_DIM} embedding)")

        set_seed(seed)
        message_pool = {}

        # Server and clients use augmented data (in_dim picked up from data.x.size(-1))
        server = FedAvgServer(args, train_list_aug[0], data_dir, message_pool, device)
        clients = [
            FedAvgClient(args, cid, train_list_aug[cid], data_dir, message_pool, device)
            for cid in range(num_clients)
        ]

        # Eval loaders use augmented val/test data
        val_loaders  = [make_eval_loader(val_list_aug[cid],  server.task, device, shuffle=True) for cid in range(num_clients)]
        test_loaders = [make_eval_loader(test_list_aug[cid], server.task, device, shuffle=True) for cid in range(num_clients)]

        model_name = f"fedavg_bootstrap_seed{seed}"
        epoch_csv_path = start_epoch_csv(
            model_name=model_name,
            seed=seed,
            tasks=TASKS,
            out_dir="./results/metrics/federated_logs/fedavg_bootstrap",
        )

        ckpt_dir = "./checkpoints/fedavg_bootstrap"
    ...
    print(f"\n{'='*60}")
    print(
        f"[Results] FedAvg Bootstrap — macro minority F1 (mean across {len(seeds)} seeds × {num_clients} clients): "
        f"{macro_mean:.2f}%"
    )
    row = " | ".join(
    ...
    out_csv     = "./results/metrics/federated_logs/fedavg_bootstrap_results.csv"
    out_csv_auc = "./results/metrics/federated_logs/fedavg_bootstrap_pr_auc_results.csv"
    os.makedirs(os.path.dirname(out_csv), exist_ok=True)

    model_name_str = (
        f"FedAvg Bootstrap PNA | "
